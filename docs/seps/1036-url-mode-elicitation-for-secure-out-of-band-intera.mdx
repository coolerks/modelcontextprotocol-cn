---
title: "SEP-1036: 用于安全带外交互的 URL 模式引出"
sidebarTitle: "SEP-1036: 用于安全带外交互的 URL 模式引出"
description: "用于安全带外交互的 URL 模式引出"
---
...
本 SEP 为现有的引出客户端能力引入了一种新的 `url` 模式，支持绕过 MCP 客户端的安全带外交互。URL 模式引出解决了表单模式引出无法处理的高敏感用例，例如收集敏感凭据、为外部（第三方）授权执行 OAuth 流程以及处理支付，同时*不会*将敏感数据暴露给 MCP 客户端。通过引导用户在浏览器中访问受信任的 URL，此模式在保持安全边界的同时，实现了与第三方服务的丰富集成。
