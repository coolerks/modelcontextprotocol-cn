...
sidebarTitle: "SEP-1613: 确立 JSON Schema 2020-12 为 MCP 的默认方言"
description: "确立 JSON Schema 2020-12 为 MCP 的默认方言"
---
...
本 SEP 将 JSON Schema 2020-12 确立为 MCP 消息中嵌入式模式的默认方言（工具的 `inputSchema`/`outputSchema` 和引出的 `requestedSchema` 字段）。模式可以通过 `$schema` 字段显式声明其他方言。这解决了导致实现之间兼容性问题的歧义。
