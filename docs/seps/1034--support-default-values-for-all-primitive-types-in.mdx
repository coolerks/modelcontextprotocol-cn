---
title: "SEP-1034: 在引出模式中支持所有原始类型的默认值"
sidebarTitle: "SEP-1034: 在引出模式中支持所有原始类型的默认值"
description: "在引出模式中支持所有原始类型的默认值"
---
...
本 SEP 建议在 MCP 引出模式（StringSchema、NumberSchema 和 EnumSchema）中为所有原始类型添加默认值支持，扩展当前仅覆盖 BooleanSchema 的现有支持。
