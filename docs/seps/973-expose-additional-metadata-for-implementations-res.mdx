---
title: "SEP-973: 为实现、资源、工具和提示暴露额外的元数据"
sidebarTitle: "SEP-973: 为实现、资源、工具和提示暴露额外的元数据"
description: "为实现、资源、工具和提示暴露额外的元数据"
---
...
本 SEP 提议添加两个可选字段——`icons` 和 `websiteUrl`。`icons` 和 `websiteUrl` 将被添加到 `Implementation` 模式中，以便客户端能够直观地识别第三方实现并直接链接到其文档。`icons` 参数也将被添加到 `Tool`、`Resource` 和 `Prompt` 模式中。虽然服务器和客户端都可以在所有实现中使用它，但我们预计它将首先用于服务器提供的实现。
