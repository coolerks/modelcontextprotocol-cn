---
title: "SEP-1330: 引出枚举模式改进与标准合规"
sidebarTitle: "SEP-1330: 引出枚举模式改进与标准合规"
description: "引出枚举模式改进与标准合规"
---
...
本 SEP 提议改进 MCP 中的枚举模式定义，弃用非标准的 `enumNames` 属性，转而采用符合 JSON Schema 的模式，并在单选模式之外引入对多选枚举模式的额外支持。新模式已根据 JSON 规范进行了验证。
