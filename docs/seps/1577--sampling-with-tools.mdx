...
本 SEP 为 `sampling/createMessage` 引入了 `tools` 和 `toolChoice` 参数，并软弃用了 `includeContext`（将 `thisServer` 和 `allServers` 限制在某个能力下）。这使得 MCP 服务器能够使用客户端的令牌运行自己的代理循环（仍在用户监督下），并降低了客户端实现的复杂性（上下文支持变为明确可选）。
