...
1. 取消通知 **MUST** 仅引用以下请求：
   - 先前以相同方向发出的请求
   - 被认为仍在进行中的请求
1. `initialize` 请求 **MUST NOT** 被客户端取消
1. 对于[任务增强型请求](./tasks)，**MUST** 使用 `tasks/cancel` 请求而不是 `notifications/cancelled` 通知。任务有其专用的取消机制，会返回最终的任务状态。
1. 取消通知的接收方 **SHOULD**：
   - 停止处理被取消的请求
   - 释放相关资源
   - 不发送被取消请求的响应
1. 接收方 **MAY** 忽略取消通知，如果：
   ...
   - 处理已经完成
   - 请求无法被取消
1. 取消通知的发送方 **SHOULD** 忽略之后到达的该请求的任何响应
