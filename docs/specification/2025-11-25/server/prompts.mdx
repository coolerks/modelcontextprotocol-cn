...
![提示作为斜杠命令显示的示例](/specification/2025-11-25/server/slash-command.png)

然而，实现者可以自由地通过任何适合其需求的界面模式来暴露提示——协议本身并不强制规定任何特定的用户交互模型。

## 能力

支持提示的服务器 **MUST** 在[初始化](/specification/2025-11-25/basic/lifecycle#initialization)期间声明 `prompts` 能力：
...
要检索可用的提示，客户端发送 `prompts/list` 请求。此操作支持[分页](/specification/2025-11-25/server/utilities/pagination)。
...
要检索特定提示，客户端发送 `prompts/get` 请求。参数可以通过[补全 API](/specification/2025-11-25/server/utilities/completion) 自动补全。
...
<Note>
  提示消息中的所有内容类型都支持可选的
  [注解](./resources#annotations)，用于提供关于受众、优先级 和修改时间的元数据。
</Note>
