...
本文档描述了 MCP 服务器向 MCP 客户端通告其关联授权服务器的机制，
以及 MCP 客户端可以确定授权服务器端点和支持的能力的发现过程。

## 授权服务器位置

MCP 服务器 **MUST** 实现 OAuth 2.0 受保护资源元数据（[RFC9728](https://datatracker.ietf.org/doc/html/rfc9728)）
规范以指示授权服务器的位置。MCP 服务器返回的受保护资源元数据文档 **MUST** 包含
至少一个授权服务器的 `authorization_servers` 字段。
...
MCP 使用 [RFC 8414 第 3.1 节](https://datatracker.ietf.org/doc/html/rfc8414#section-3.1) 中定义的默认 `oauth-authorization-server` well-known URI 后缀进行授权服务器元数据发现。MCP 未定义应用程序特定的 well-known URI 后缀。
