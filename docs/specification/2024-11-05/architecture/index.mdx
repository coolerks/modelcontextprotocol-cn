...
- **请求（Requests）**：带有方法和参数的双向消息，期望得到响应
- **响应（Responses）**：成功结果或错误，匹配特定的请求 ID
- **通知（Notifications）**：不需要响应的单向消息

每种消息类型遵循 JSON-RPC 2.0 规范关于结构和传递语义的要求。
...
- 实现的[服务器特性](/specification/2024-11-05/server) 必须在服务器的能力中声明
- 发出资源订阅通知要求服务器声明订阅支持
- 工具调用要求服务器声明工具能力
- [采样](/specification/2024-11-05/client/sampling) 要求客户端在其能力中声明支持
